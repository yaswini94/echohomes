...
describe('BudgetBasedSuggestions Component', () => {
  ...
  test('displays "No Extras are in budget!" when no extras are within the budget', async () => {
    const mockChoices = [{ feature_id: 'choice-1', name: 'Choice 1', price: 3000 }];
    const mockExtras = [{ feature_id: 'extra-1', name: 'Extra 1', price: 15000 }]; // Extras are above the budget

    // Mock the API responses
    axiosInstance.get
      .mockResolvedValueOnce({ data: { buyer_id: 'buyer-id', venture_id: 'venture-id' } }) // Buyer API response
      .mockResolvedValueOnce({
        data: { venture_id: 'venture-id', properties: [{ key: 'house_type', choices: ['choice-1'], extras: ['extra-1'] }] }, // Venture API response
      })
      .mockResolvedValueOnce({
    ...
    // Enter a budget
    fireEvent.change(screen.getByPlaceholderText('Enter your budget'), { target: { value: 1000 } }); // Budget less than extra prices
